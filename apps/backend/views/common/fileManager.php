<div class="modal-dialog modal-lg">
    <div class="modal-content">
        ...
        <div class="modal-body">
            <div class="row">
                <div class="col-sm-5">
                    <a href="{{ parent }}" data-toggle="tooltip" title="上一级" id="button-parent" class="btn btn-default">
                        <i class="fa fa-level-up"></i>
                    </a>
                    <a href="{{ refresh }}" data-toggle="tooltip" title="刷新" id="button-refresh" class="btn btn-default">
                ...
                <div class="col-sm-7">
                    <div class="input-group">
                        <input type="text" name="search" value="<?php echo $filter_name; ?>" placeholder="搜索中..." class="form-control">
                        <span class="input-group-btn">
                        <button type="button" data-toggle="tooltip" title="检索" id="button-search" class="btn btn-primary"><i class="fa fa-search"></i></button>
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-footer pagination"><?php echo $pagination; ?></div>
    </div>
</div>
<script type="text/javascript">
    ...
    $('#button-search').on('click', function(e) {
        var url = '{{url({"for" : "backend/common/fileManager"})}}?directory=<?php echo $directory; ?>';
...
<script type="text/javascript">
    $('#button-upload').on('click', function() {
        ...
        timer = setInterval(function() {
            if ($('#form-upload input[name=\'file\']').val() != '') {
                ...
                $.ajax({
                    url: '{{url({"for" : "backend/common/fileUpload"})}}?directory=<?php echo $directory; ?>',
                    type: 'post',
                    dataType: 'json',
    ...
    $('#button-folder').on('shown.bs.popover', function() {
        $('#button-create').on('click', function() {
            $.ajax({
                url: '{{url({"for" : "backend/common/fileFolder"})}}?directory=<?php echo $directory; ?>',
                type: 'post',
                dataType: 'json',
    ...
    $('#modal-image #button-delete').on('click', function(e) {
        if (confirm('确定！')) {
            $.ajax({
                url: '{{url({"for" : "backend/common/fileDelete"})}}',
                type: 'post',
                dataType: 'json',
