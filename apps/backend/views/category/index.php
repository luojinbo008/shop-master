...
<script type="text/javascript">
    // 绑定删除事件
    $("#backend-category-delete").click(function () {
    ...
    var TableManaged = function () {
        return {
            init: function () {
                ...
                $('#sample_1').dataTable({
                    "aoColumns": [
                        { "mDataProp": "category_id", "sClass" : "text-center", "bSortable": false, "fnRender": function(obj) {
                            obj.aData.edit_id = obj.aData.category_id;
                            var html = '<input type="checkbox" name="selected[]" value="' + obj.aData.category_id + '" />';
                            return html;
                        }},
                        ...
                        { "mDataProp": "edit_id", "sClass" : "text-center", "bSortable": false, "fnRender": function(obj) {
                            ...
                            {% for child in currentMenu[currentMenu|length - 1]['children']['table'] %}
                                html = "<a href=\"{{ url({'for' : child['for']}) }}?category_id="
                    ...
                    "bPaginate" : true,
                    "bDestroy":true,
                    "sAjaxSource" : "{{ url({'for': 'backend/category'}) }}",
    ...
    function del(){
          $("#form-category").ajaxSubmit({
              type:"post",
              url:"{{ url({'for' : 'backend/category/delete'}) }}",
